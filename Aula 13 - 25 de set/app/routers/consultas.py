...
@router.get("/{consulta_id}", response_model=ConsultaPublicDetalhada)
def buscar_consulta(consulta_id: int, session: Session = Depends(getSession)):
...
def atualizarConsulta(consulta_id: int, dados: ConsultaUpdate, session: Session = Depends(getSession)):
    ...
    paciente_id = valores.get(
        "paciente_id",
        consulta.paciente_id,
    )
    
    medico_id = valores.get(
        "medico_id",
        consulta.medico_id,
    )
    
    especialidade_id = valores.get(
        "especlidade_id",
        consulta.especlidade_id,
    )
...
def apagarConsulta(consulta_id: int, session: Session = Depends(getSession)):
