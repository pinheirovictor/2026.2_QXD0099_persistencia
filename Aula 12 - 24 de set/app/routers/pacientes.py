...
def buscar_paciente(paciente_id: int, session: Session = Depends(getSession),):
...
def atualizar_paciente(paciente_id: int, dados: PacienteUpdate, session: Session = Depends(getSession),):
    paciente = session.get(
        Paciente, 
        paciente_id
    )
    ...
    paciente.sqlmodel_update(dados_atualizacao)
...
def apagar_paciente(paciente_id: int, session: Session = Depends(getSession),):
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail=("Paciente possui consultas associadas a ele, e não pdoe ser removido")
        )
